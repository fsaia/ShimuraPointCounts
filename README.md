...
- `unknown_aut_star_pairs_10k.m`: List of all $1321$ of the $10609$ pairs $(D,N)$ with $D>1$ an indefinite quaternion discriminant over $\mathbb{Q}$, $N$ a positive integer which is squarefree and coprime to $D$, $g(X\_0^D(N)^*) > 2$, and $DN < 10000$ for which we do not determine in this work whether $X_0^D(N)^*$ has a non-trivial involution. Computed in `aut_checks.m`.  
