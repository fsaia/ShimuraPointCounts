# ShimuraPointCounts
Magma code for the paper *Point counts, automorphisms, and gonalities of Shimura curves* by Pietro Mercuri, Oana Padurariu, Frederick Saia, and Claudio Stirpe ([arXiv:2507.15992](https://arxiv.org/abs/2507.15992))
...
This directory contains code relevant to proving Theorem 1.3 in our paper (see also Section 5). To run all of these checks, one will need the files from the Finite Field Point Counts directory as well as the auxiliary file `quot_genus.m`.
...
- `no_involution_star_pairs_10k.m`: List of all $9288$ of the $10609$ pairs $(D,N)$ with $D>1$ an indefinite quaternion discriminant over $\mathbb{Q}$, $N$ a positive integer which is squarefree and coprime to $D$, $g(X_0^D(N)^*) > 2$, and $DN < 10000$ for which we prove that $X_0^D(N)^*$ has no non-trivial involutions using point counts with the result of González. Computed in `aut_checks.m`. This file is needed for `tetragonal_sieving.m`.

- `unknown_aut_star_pairs_10k.m`: List of all $1321$ of the $10609$ pairs $(D,N)$ with $D>1$ an indefinite quaternion discriminant, $N$ a positive integer which is squarefree and coprime to $D$, $g(X_0^D(N)^*) > 2$, and $DN < 10000$ for which we have not yet determined whether $X_0^D(N)^*$ has a non-trivial involution. Computed in `aut_checks.m`.
...
- `unknown_aut_pairs_10k.m`: List of all $12$ of the $10609$ pairs $(D,N)$ with $D>1$ an indefinite quaternion discriminant, $N$ a positive integer which is squarefree and coprime to $D$, $g(X_0^D(N)^*) > 2$, and $DN < 10000$ for which we have not yet determined whether $X_0^D(N)$ has a non-Atkin--Lehner involution. Computed in `aut_checks.m`.  


## Tetragonals

This directory contains code relevant to proving Theorem 1.5 in our paper (see also Section 6). To run all of these checks, one will need the files from the Finite Field Point Counts directory, the file `no_involution_star_pairs_10k.m` from the Automorphisms directory, and all files from the Auxiliary Files directory.
