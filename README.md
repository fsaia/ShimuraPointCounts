...
- `no_involution_star_pairs_10k.m`: List of all $9288$ of the $10609$ pairs $(D,N)$ with $D>1$ an indefinite quaternion discriminant over $\mathbb{Q}$, $N$ a positive integer which is squarefree and coprime to $D$, $g(X_0^D(N)) > 2$, and $DN < 10000$ for which we prove that $X_0^D(N)^*$ has no non-trivial involutions using point counts with the result of $\text{Gonz\'{a}lez}$. Computed in `aut_checks.m`. This file is needed for `tetragonal_sieving.m`. 
